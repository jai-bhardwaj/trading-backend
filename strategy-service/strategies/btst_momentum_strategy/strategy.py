"""
BTST Momentum Strategy - Migrated to new architecture
Buy Today Sell Tomorrow with momentum
"""
import asyncio
import logging
import os
...
class BTSTMomentumStrategy(BaseStrategy):
    """BTST Momentum Strategy using new architecture"""
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.holding_days = self.parameters.get('holding_days', 1)
        
        logger.info(f"BTST Momentum Strategy initialized with parameters:")
        ...
        for symbol in self.symbols:
            ...
            try:
                ...
                if momentum >= self.momentum_percentage:
                    ...
                    logger.info(f"📈 BTST Momentum BUY signal for {symbol}: Momentum={momentum:.2f}%")
                    
            except Exception as e:
                logger.error(f"❌ Error in BTST Momentum strategy for {symbol}: {e}")
...
async def main():
    ...
    strategy_id = os.getenv('STRATEGY_ID', 'btst_momentum_strategy')
    ...
    parameters = {
        ...
        'holding_days': int(os.getenv('HOLDING_DAYS', '1')),
    ...
    strategy = BTSTMomentumStrategy(config)
    
    try:
        logger.info(f"🚀 Starting BTST Momentum Strategy Service...")
        success = await strategy.start()
        
        if success:
            logger.info(f"✅ BTST Momentum Strategy Service started successfully")
            ...
            logger.error(f"❌ Failed to start BTST Momentum Strategy Service")
        ...
        await strategy.stop()
        logger.info("✅ BTST Momentum Strategy Service stopped")

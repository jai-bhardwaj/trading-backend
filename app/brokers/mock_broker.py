...
class MockBroker(BrokerInterface):
    ...
    _user_data: Dict[str, Dict[str, Any]] = (
        {}
    )  # { user_id: {"orders": {}, "positions": {}, "balance": {}} }

    def __init__(self):
        self._initialized_users: set[str] = (
            set()
        )  # Track initialized users for this instance
    ...
    async def initialize(self, user_id: str, config: Dict[str, Any]):
        ...
        if user_id not in MockBroker._user_data:
            ...
            logger.info(
                f"Created initial mock data for user {user_id} with balance {initial_balance}"
            )
        else:
    ...
    def _get_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        if user_id not in MockBroker._user_data:
            ...
            raise RuntimeError(
                f"Mock data structure not found for initialized user {user_id}"
            )
    ...
    async def place_order(self, order: Order) -> Dict[str, Any]:
        ...
        logger.info(
            f"MockBroker: Received order {order.internal_order_id} for user {order.user_id}: {order.side} {order.quantity} {order.symbol} @ {order.order_type}"
        )
        ...
        if order.order_type == "LIMIT" and order.price is None:
            ...
            user_data["orders"][order.internal_order_id] = copy.deepcopy(
                order
            )  # Store rejected order
            logger.warning(
                f"MockBroker: Order {order.internal_order_id} REJECTED: {order.status_message}"
            )
            return {
        ...
        return {
            "broker_order_id": order.internal_order_id,
            "status": order.status,
            "message": "Order accepted by mock broker",
        }
    ...
    async def _simulate_fill(self, internal_order_id: str, user_id: str):
        ...
        try:
            ...
            logger.error(
                f"MockBroker: User {user_id} data not found during fill simulation for {internal_order_id}."
            )
            return
        ...
        if internal_order_id not in orders:
            logger.warning(
                f"MockBroker: Order {internal_order_id} not found for fill simulation (maybe cancelled?)."
            )
            return
        ...
        if order.status not in ["ACCEPTED", "OPEN"]:  # Add more states if needed
            logger.info(
                f"MockBroker: Order {internal_order_id} not in fillable state ({order.status}). Skipping fill."
            )
            return
        ...
        if order.order_type == "MARKET":
            ...
            if (order.side == "BUY" and order.price >= simulated_market_price) or (
                ...
                logger.info(
                    f"MockBroker: Limit order {internal_order_id} ({order.price}) not filled at simulated price {simulated_market_price:.2f}"
                )
        ...
        logger.info(
            f"MockBroker: Order {internal_order_id} COMPLETE! Qty: {fill_qty}, AvgPrice: {fill_price:.2f}"
        )
        ...
        symbol = (
            order.symbol
        )  # Use the full symbol including exchange if needed for uniqueness
        ...
        if order.side == "BUY":
            ...
            if new_qty != 0:
                new_avg = (
                    (current_avg * current_qty) + (fill_price * fill_qty)
                ) / new_qty
            else:
            ...
            if new_qty != 0:
                ...
                if (current_qty > 0 and new_qty > 0) or (
                    ...
                    new_avg = (
                        current_avg  # Avg price remains same when reducing long/short
                    )
                elif new_qty == 0:
        ...
        current_pos.average_price = (
            new_avg if abs(new_qty) > 1e-9 else 0.0
        )  # Handle potential float issues near zero
        ...
        )  # In reality, add portfolio value

        logger.info(
            f"MockBroker: User {user_id} New Position {symbol}: Qty={new_qty}, AvgPx={new_avg:.2f}, Balance: {balance.available_cash:.2f}"
        )
    ...
    async def cancel_order(
        ...
        internal_order_id = (
            broker_order_id  # In mock, broker_order_id is our internal ID
        )

        logger.info(
            f"MockBroker: Received cancel request for order {internal_order_id}"
        )
        if internal_order_id in user_data["orders"]:
            ...
            if order.status in [
                ...
                logger.warning(
                    f"MockBroker: Order {internal_order_id} cannot be cancelled in state {order.status}."
                )
                return {
            ...
            logger.error(
                f"MockBroker: Order {internal_order_id} not found for cancellation."
            )
    ...
    async def get_order_status(
        ...
        if internal_order_id in user_data["orders"]:
            ...
            return (
                order.__dict__
            )  # Simple approach for mock; could map to a cleaner dict
        else:
            logger.warning(
                f"MockBroker: Order {internal_order_id} not found for user {user_id}"
            )

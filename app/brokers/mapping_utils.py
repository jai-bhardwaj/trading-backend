...
"""
from app.core.interfaces import Order, Position, Balance
import logging
...
def map_order_to_angelone(order: Order) -> dict[str, any]:
    ...
    params = {
        "variety": variety_map.get(order.variety, "NORMAL"), # Default to NORMAL if not specified or map based on order_type/product_type
    ...
    if order.order_type == "LIMIT":
        if order.price is None: raise ValueError("Price needed for LIMIT order")
        params["price"] = str(order.price)
    elif order.order_type == "SL":
        if order.price is None or order.trigger_price is None: raise ValueError("Price and Trigger Price needed for SL order")
        params["price"] = str(order.price)
        ...
        if order.trigger_price is None: raise ValueError("Trigger Price needed for SL-M order")
        params["triggerprice"] = str(order.trigger_price)
    ...
    logger.warning(f"SYMBOL TOKEN for {order.symbol} is NOT being fetched. Order placement will likely fail.") # Add this warning
    ...
    if not params["transactiontype"] or not params["ordertype"] or not params["producttype"]:
        raise ValueError(f"Order mapping failed for side={order.side}, order_type={order.order_type}, product_type={order.product_type}")
...
def map_angelone_positions(api_positions_data: list[dict[str, any]]) -> list[Position]:
    ...
    for p in api_positions_data:
        try:
            ...
            if qty == 0: continue # Skip zero positions
            ...
            pos = Position(
                ...
                quantity=qty,
                average_price=float(p.get('netavgprice', 0.0)), # Or buyavgprice/sellavgprice? Use netavgprice if available.
                last_traded_price=float(p.get('ltp', 0.0)),
                pnl=float(p.get('unrealised', 0.0)) # Or 'realised'? check PnL field names
                # Add other relevant fields from Angel One position data
            )
            positions.append(pos)
        except (ValueError, TypeError, KeyError) as e:
            logger.error(f"Error mapping Angel One position: {p}. Error: {e}", exc_info=True)
...
def map_angelone_balance(api_profile_data: dict[str, any]) -> Balance:
    ...
    try:
         ...
         rms_data = api_profile_data # Or api_profile_data.get('rms') depending on structure
         balance = Balance(
             ...
             margin_used=float(rms_data.get('utilisedmargin', 0.0)), # Check correct field
             # Map total balance/equity (check fields like 'net', 'equity', 'collateral')
             total_balance=float(rms_data.get('net', 0.0)) # Check correct field for overall balance/equity
             # Map other relevant fields like collateral, exposure limits etc.
         )
         return balance
    except (ValueError, TypeError, KeyError) as e:
        logger.error(f"Error mapping Angel One balance from data: {api_profile_data}. Error: {e}", exc_info=True)
...
def map_angelone_order_status(api_order_data: dict[str, any]) -> dict[str, any]:
    ...
    try:
        ...
        return {
            ...
            "pending_quantity": int(api_order_data.get('unfilledshares', 0)), # If available
            ...
            "status_message": api_order_data.get('text') or api_order_data.get('statusmessage'), # Check field name for rejection/status message
            "order_timestamp": api_order_data.get('orderentrytime') or api_order_data.get('updatetime'), # Check timestamp field name(s)
            "variety": api_order_data.get('variety'),
            # Add any other relevant fields: disclosed_quantity, tag, etc.
        }
    except (ValueError, TypeError, KeyError) as e:
         logger.error(f"Error mapping Angel One order status: {api_order_data}. Error: {e}", exc_info=True)
         return {"broker_order_id": api_order_data.get('orderid'), "status": "MAPPING_ERROR", "error": str(e)}

def map_angelone_trade_history(api_trade_data: list[dict[str, any]]) -> list[dict[str, any]]:
    ...
    for t in api_trade_data:
        try:
            ...
            trade = {
                "trade_id": t.get("tradeid") or f"{t.get('orderid')}_{t.get('fillid')}", # Need a unique ID
                "broker_order_id": t.get("orderid"),
                "symbol": t.get("tradingsymbol"),
                "exchange": t.get("exchange"),
                "side": t.get("transactiontype"), # BUY/SELL
                "quantity": int(t.get("fillsize") or t.get("quantity")), # Check field name for fill quantity
                "price": float(t.get("fillprice") or t.get("price")), # Check field name for fill price
                "product_type": t.get("producttype"),
                "order_type": t.get("ordertype"),
                 "trade_timestamp": t.get("filltime") or t.get("tradetime"), # Check timestamp field
                 # Add other fields if available: charges, segment, etc.
            }
            trades.append(trade)
        except (ValueError, TypeError, KeyError) as e:
            logger.error(f"Error mapping Angel One trade: {t}. Error: {e}", exc_info=True)
            continue
    return trades

...
_user_sessions: Dict[str, Dict[str, Any]] = {} # { user_id: {"smartApi": SmartConnect, "session_data": {}, "lock": asyncio.Lock} }
...
class AngelOneBroker(BrokerInterface):
    ...
    async def _get_smartapi_instance(self, user_id: str, config: Dict[str, Any]) -> SmartConnect:
        """
        ...
        async with user_session["lock"]: # Acquire lock before checking/initializing
            ...
            if not all([api_key, client_id, password, totp_secret]):
                raise ValueError(f"Missing required Angel One credentials in config for user {user_id}")
            ...
            try:
                ...
                session_data = await asyncio.to_thread(self._perform_login, smartApi, client_id, password, totp_secret)
                # ----------------------------------------------

                if not session_data or 'jwtToken' not in session_data:
                     raise ConnectionError("Failed to retrieve valid session data from Angel One.")
                ...
                logger.info(f"Angel One session established successfully for user {user_id}.")
                return smartApi

            except Exception as e:
                logger.error(f"Failed to initialize Angel One session for user {user_id}: {e}", exc_info=True)
                # Clear potentially bad state
                user_session["smartApi"] = None
                user_session["session_data"] = None
                raise ConnectionError(f"Angel One initialization/login failed for user {user_id}: {e}")
        # Lock released automatically

    def _perform_login(self, smartApi: SmartConnect, client_id: str, password: str, totp_secret: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Generated TOTP for Angel One login (client {client_id}).") # Careful with logging TOTP
            data = smartApi.generateSession(client_id, password, totp)

            if data.get("status") and data.get("data"):
                 logger.info(f"Angel One session generated successfully for {client_id}.")
                 return data["data"]
            else:
                 error_message = data.get("message", "Unknown login error")
                 error_code = data.get("errorcode", "N/A")
                 logger.error(f"Angel One login failed for {client_id}: {error_message} (Code: {error_code})")
                 # Map specific error codes if needed (e.g., AB1010 invalid TOTP)
                 raise ConnectionError(f"Angel One login failed ({error_code}): {error_message}")

        except Exception as e:
            logger.error(f"Exception during Angel One login execution for {client_id}: {e}", exc_info=True)
    ...
    async def initialize(self, user_id: str, config: Dict[str, Any]):
        ...
        await self._get_smartapi_instance(user_id, config) # This will trigger login if needed
        logger.info(f"AngelOneBroker connection check/initialization complete for user {user_id}.")


    async def _run_sync_api_call(self, user_id: str, func_name: str, *args, **kwargs) -> Any:
        """Helper to run synchronous SDK methods safely in a thread pool."""
        smartApi = await self._get_smartapi_instance(user_id, _user_sessions[user_id]['config']) # Get potentially refreshed instance
        ...
        try:
            ...
            if isinstance(result, dict):
                if result.get("status") == True or str(result.get("status")).lower() == 'success':
                    logger.debug(f"Angel One API call {func_name} successful for user {user_id}.")
                    ...
                    logger.error(f"Angel One API call {func_name} failed for user {user_id}: {error_message} (Code: {error_code})")
                    ...
                    raise ConnectionAbortedError(f"Angel One API Error ({error_code}): {error_message}")
            elif result is None:
                 # Some API calls might return None on failure or success? Check docs.
                 logger.warning(f"Angel One API call {func_name} returned None for user {user_id}. Args: {args}, Kwargs: {kwargs}")
                 return None # Or raise error depending on expected behavior
            else:
                 # Handle cases where the response might not be the expected dict
                 logger.warning(f"Unexpected response format from {func_name} for user {user_id}: {type(result)} - {result}")
                 return result # Or raise an error

        except Exception as e:
            # Catch errors during asyncio.to_thread or within the API call execution
            logger.error(f"Error during Angel One API call {func_name} execution for user {user_id}: {e}", exc_info=True)
            # Check for specific SDK exceptions if available
            raise ConnectionError(f"Failed to execute Angel One API call {func_name} for user {user_id}: {e}")
    ...
    async def place_order(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Placing Angel One order for user {order.user_id}: Params={order_params}")

            # Ensure symboltoken is present (mapping should handle this, add check)
            if not order_params.get("symboltoken"):
                 logger.error(f"Missing 'symboltoken' for order: {order.symbol}. Implement token fetching in mapping_utils.")
                 raise ValueError(f"Cannot place order for {order.symbol} without a valid 'symboltoken'.")

            result_data = await self._run_sync_api_call(order.user_id, "placeOrder", order_params)

            broker_order_id = result_data.get('orderid') if isinstance(result_data, dict) else None

            if broker_order_id:
                logger.info(f"Angel One order placed successfully for user {order.user_id}. Broker Order ID: {broker_order_id}")
                return {"broker_order_id": broker_order_id, "status": "PLACED", "message": "Order placed successfully."} # Status might be different initially
            else:
                logger.error(f"Angel One place order call succeeded but did not return an order ID. User: {order.user_id}, Response data: {result_data}")
                return {"broker_order_id": None, "status": "UNKNOWN", "message": "Order placement response did not contain order ID."}

        except (ValueError, ConnectionAbortedError, ConnectionError) as e:
             # Catch mapping errors or API errors from _run_sync_api_call
             logger.error(f"Failed to place Angel One order for user {order.user_id}: {e}", exc_info=True)
             return {"broker_order_id": None, "status": "ERROR", "message": f"Failed to place order: {e}"}
        except Exception as e:
             # Catch unexpected errors
             logger.exception(f"Unexpected error placing Angel One order for user {order.user_id}: {e}")
             return {"broker_order_id": None, "status": "ERROR", "message": f"Unexpected error: {e}"}


    async def cancel_order(self, user_id: str, broker_order_id: str, variety: Optional[str] = "NORMAL") -> Dict[str, Any]:
        ...
        if not variety:
             logger.warning("Variety not provided for cancel order, defaulting to 'NORMAL'. This might fail for AMO/Special orders.")
             variety = "NORMAL" # Default, but might be wrong!

        logger.info(f"Attempting to cancel Angel One order {broker_order_id} (Variety: {variety}) for user {user_id}")
        try:
            # Params expected: variety, orderid
            result_data = await self._run_sync_api_call(user_id, "cancelOrder", variety, broker_order_id)

            cancelled_order_id = result_data.get('orderid') if isinstance(result_data, dict) else None

            if cancelled_order_id == broker_order_id:
                 logger.info(f"Angel One cancel order request successful for order {broker_order_id}, user {user_id}.")
                 # The actual status change needs confirmation via order book/update
                 return {"broker_order_id": broker_order_id, "status": "CANCEL_REQUESTED", "message": "Cancel request sent."}
            else:
                 # This might occur if the API call succeeds but confirms differently, or structure changes
                 logger.warning(f"Angel One cancel order response for {broker_order_id} had unexpected data: {result_data}. Assuming success if no exception.")
                 # Check if the error message indicates it was already cancelled/completed
                 return {"broker_order_id": broker_order_id, "status": "CANCEL_REQUESTED", "message": "Cancel request sent (response structure unexpected)."}

        except (ValueError, ConnectionAbortedError, ConnectionError) as e:
             logger.error(f"Failed to cancel Angel One order {broker_order_id} for user {user_id}: {e}", exc_info=True)
             # Check error message for common reasons like "order already completed/cancelled"
             message = f"Failed to cancel order: {e}"
             status = "ERROR"
             if "already" in str(e).lower() and ("completed" in str(e).lower() or "cancelled" in str(e).lower() or "rejected" in str(e).lower()):
                  status = "ALREADY_CLOSED" # Or fetch status to be sure
                  message = f"Order likely already closed: {e}"
                  logger.warning(f"Cancel failed for {broker_order_id}, likely already closed.")

             return {"broker_order_id": broker_order_id, "status": status, "message": message}
        except Exception as e:
             logger.exception(f"Unexpected error cancelling Angel One order {broker_order_id} for user {user_id}: {e}")
             return {"broker_order_id": broker_order_id, "status": "ERROR", "message": f"Unexpected error: {e}"}
    ...
    async def get_order_status(self, user_id: str, broker_order_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Fetching Angel One order status for order {broker_order_id}, user {user_id}")
        try:
            ...
            if isinstance(order_book_data, list):
                for order_detail in order_book_data:
                    if isinstance(order_detail, dict) and order_detail.get('orderid') == broker_order_id:
                        logger.info(f"Found order {broker_order_id} in order book for user {user_id}.")
                        return map_angelone_order_status(order_detail) # Map the found order
                # Order not found in the list
                logger.warning(f"Order {broker_order_id} not found in Angel One order book for user {user_id}.")
                return {"broker_order_id": broker_order_id, "status": "NOT_FOUND", "message": "Order not found in today's order book."}
            else:
                 logger.error(f"Unexpected data type received from Angel One orderBook for user {user_id}: {type(order_book_data)}")
                 raise ValueError("Unexpected data format from orderBook API.")

        except (ValueError, ConnectionAbortedError, ConnectionError) as e:
            logger.error(f"Failed to get Angel One order status for {broker_order_id}, user {user_id}: {e}", exc_info=True)
            return {"broker_order_id": broker_order_id, "status": "ERROR", "message": f"Failed to get order status: {e}"}
        except Exception as e:
            logger.exception(f"Unexpected error getting Angel One order status for {broker_order_id}, user {user_id}: {e}")
            return {"broker_order_id": broker_order_id, "status": "ERROR", "message": f"Unexpected error: {e}"}
    ...
    async def get_order_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            order_book_data = await self._run_sync_api_call(user_id, "orderBook")
            if isinstance(order_book_data, list):
                return [map_angelone_order_status(od) for od in order_book_data if isinstance(od, dict)]
            else:
                 logger.error(f"Unexpected data type received from Angel One orderBook for user {user_id}: {type(order_book_data)}")
                 return [] # Return empty list on unexpected format
        except Exception as e:
            logger.error(f"Failed to get Angel One order history for user {user_id}: {e}", exc_info=True)
    ...
    async def get_positions(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            elif isinstance(position_data, dict) and 'data' in position_data and isinstance(position_data['data'], list):
                 # Handle if data is nested under a 'data' key
                 return [pos.__dict__ for pos in map_angelone_positions(position_data['data'])]
            else:
                 logger.error(f"Unexpected data type received from Angel One position API for user {user_id}: {type(position_data)}")
                 return []
        except Exception as e:
            logger.error(f"Failed to get Angel One positions for user {user_id}: {e}", exc_info=True)
            return []
    ...
    async def get_balance(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            profile_data = await self._run_sync_api_call(user_id, "getProfile", None) # Pass profile request object if needed by SDK
            ...
            if isinstance(profile_data, dict):
                 ...
                 logger.error(f"Unexpected data type received from Angel One getProfile API for user {user_id}: {type(profile_data)}")
                 # Return a default/error structure
                 return Balance(available_cash=0, margin_used=0, total_balance=0).__dict__
        except Exception as e:
            logger.error(f"Failed to get Angel One balance for user {user_id}: {e}", exc_info=True)
            return Balance(available_cash=0, margin_used=0, total_balance=0).__dict__ # Return default on error
    ...
    async def get_trade_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(trade_book_data, list):
                 ...
                 logger.error(f"Unexpected data type received from Angel One tradeBook for user {user_id}: {type(trade_book_data)}")
                 return []
        except Exception as e:
            logger.error(f"Failed to get Angel One trade history for user {user_id}: {e}", exc_info=True)
            return []

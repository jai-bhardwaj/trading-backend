...
from app.api.models.portfolio import ( # Import portfolio API models
    PositionsResponse, PositionModel,
    BalanceResponse, BalanceModel,
    TradeHistoryResponse, TradeModel
)
from app.core.interfaces import BrokerInterface
from app.api.dependencies import get_resolved_broker, get_broker_details_from_query # Import dependencies
...
async def get_user_positions(
    ...
    logger.info(f"Received get positions request for user {user_id} via {broker.__class__.__name__}")
...
async def get_user_balance(
    ...
    logger.info(f"Received get balance request for user {user_id} via {broker.__class__.__name__}")
...
async def get_user_trade_history(
    ...
    user_id = broker_details["user_id"]
    broker_id = broker_details["broker_id"]
    logger.info(f"Received get trade history request for user {user_id} via {broker.__class__.__name__}")
    ...
    try:
        ...
        for trade_data in trades_list:
             trade_data['trade_timestamp'] = str(trade_data.get('trade_timestamp')) # Convert timestamp
        ...
        logger.exception(f"Unexpected error getting trade history for user {user_id}: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unexpected server error occurred: {e}")
...
from config import logger, settings  # Import logger from config
...
async def get_broker_instance(
    ...
    logger.debug(
        f"Requesting broker instance. User: {user_id}, Requested Broker: {broker_id}, Paper: {is_paper_trade}. Using: {actual_broker_id} (Key: {instance_key})"
    )
    ...
    async with lock:
        ...
        logger.info(
            f"No cached instance found for key {instance_key}. Creating and initializing..."
        )
        ...
        try:
            if actual_broker_id == "mock_broker":
                ...
                if not all(
                    broker_config.values()
                ):  # Basic check if any value is None/empty
                    logger.error(
                        f"Cannot initialize Angel One for user {user_id}: Missing credentials in configuration."
                    )
                    raise ValueError(
                        "Angel One credentials missing in server configuration."
                    )
            ...
            #     instance = OtherBroker()
                ...
                raise HTTPException(
                    status_code=501,
                    detail=f"Broker type '{actual_broker_id}' is not implemented.",
                )
            ...
            logger.info(
                f"Successfully initialized and cached broker instance for key: {instance_key}"
            )
            return instance
            ...
            logger.error(
                f"Failed to initialize broker for key {instance_key}: {e}",
                exc_info=True,
            )
            ...
            logger.exception(
                f"Unexpected error initializing broker for key {instance_key}: {e}"
            )
            raise HTTPException(
...
async def get_broker_details_from_body(
    ...
    if not user_id or not broker_id:
        raise HTTPException(
            ...
            detail="Missing 'user_id' or 'broker_id' in request body.",
        )
...
async def get_resolved_broker(
    broker_details: Dict[str, Any] = Depends(
        get_broker_details_from_query
    ),  # Default to query, override in endpoint if needed
) -> BrokerInterface:
    """
    ...
    try:
        ...
        logger.exception(
            f"Error resolving broker instance for details {broker_details}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="Internal server error resolving broker."
        )
